...
class PublicIjazah extends React.Component {
  state = {
    loading : true,
    element : null,
    modal:false,
  ...
  constructor(props){
    ...
    let nik = this.props.match.params.nik
    axios.get(url + "/ijazah/" + nik).then(res => {
      ...
      if (err.response.status === 403){
        ...
        this.setState({
            ...
            element: (
                <div>
                    <h5>Siswa dengan nik {nik} tidak mengaktifkan link Ijazah.</h5>
                </div>
            )
        })
      }
